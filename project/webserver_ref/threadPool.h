...
const uint TASK_CAP_DEFAULT = 512;
...
public:
	...
	void start(int initThreadNum = std::thread::hardware_concurrency() - 1);
...
{
	isPoolRunning_ = false;
	//notEmpty_.notify_one();//条件没变，但需唤醒线程，可能造成死锁
...
inline void ThreadPool::threadFunc(uint threadId)
{
	...
	while (true) {//线程会反复准备执行任务，属于是生产队的驴
		Task task;
		{
...
#endif
					exitCond_.notify_one();//配套V操作：我走了 我告诉你，你条件满足则执行否则继续等
					return;
				}
			...
			//V操作，有P就要有V
			if (taskQue_.size() > 0) notEmpty_.notify_one(); //??????????
